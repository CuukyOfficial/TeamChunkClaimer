...
public class TeamMainMenu extends SuperInventory {
	...
	public boolean onOpen() {
		linkItemTo(11, new ItemBuilder().playername(player.getTeam().getOwner().getName()).displayname("§5Mitgleider").buildSkull(), new Runnable() {
		...
		if (player.getTeam().getMemberType(player) != TeamMemberType.MEMBER)
			linkItemTo(13, new ItemBuilder().itemstack(Materials.COMMAND_BLOCK.parseItem()).displayname("§cEinstellungen").build(), new Runnable() {
		...
		linkItemTo(15, new ItemBuilder().itemstack(Materials.GRASS_BLOCK.parseItem()).displayname("§a" + player.getTeam().getClaimedChunks().size() + "§7/" + (player.getTeam().hasMaximumChunksReached() ? "§c" : "§a") + player.getTeam().getAllowedChunkAmount() + " §aChunks §7geclaimt").build(), new Runnable() {
